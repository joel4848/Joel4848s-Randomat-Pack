...
REWARD.Name = "Damage Bonus"
REWARD.Id = "damagebonus"

local damagebonus_bonus = CreateConVar("rdmt_joel4848_rewardpunish_damagebonus_bonus", "0.5", FCVAR_NONE, "Outgoing damage bonus.", 0, 1)

local hookIds = {}

function REWARD:Apply(target)
    -- Generate a unique ID for this pairing and save it to be cleaned up later
    local hookId = "Rdmt_Joel4848_RewardPunish_DamageBonus_" .. target:SteamID64()
...
function REWARD:CleanUp(ply)
    if ply then
        local hookId = "Rdmt_Joel4848_RewardPunish_DamageBonus_" .. ply:SteamID64()
